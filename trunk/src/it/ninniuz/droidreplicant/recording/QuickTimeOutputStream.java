package it.ninniuz.droidreplicant.recording;
...
public class QuickTimeOutputStream {
    ...
    private class CompositeAtom extends Atom {
        ...
        public void finish() throws IOException {
            if (!finished) {
                if (size() > 0xffffffffL) {
                    throw new IOException("CompositeAtom \"" + type + "\" is too large: " + size()); //$NON-NLS-1$ //$NON-NLS-2$
                }
    ...
    protected class DataAtom extends Atom {
        ...
        public DataAtomOutputStream getOutputStream() {
            if (finished) {
                throw new IllegalStateException("DataAtom is finished"); //$NON-NLS-1$
            }
            return data;
        }
        ...
        public void finish() throws IOException {
            if (!finished) {
                ...
                if (size() > 0xffffffffL) {
                    throw new IOException("DataAtom \"" + type + "\" is too large: " + size()); //$NON-NLS-1$ //$NON-NLS-2$
                }
                ...
                if (sizeBefore != sizeAfter) {
                    System.err.println("size mismatch " + sizeBefore + ".." + sizeAfter); //$NON-NLS-1$ //$NON-NLS-2$
                }
            }
        }
    ...
    protected class WideDataAtom extends Atom {
        ...
        public DataAtomOutputStream getOutputStream() {
            if (finished) {
                throw new IllegalStateException("Atom is finished"); //$NON-NLS-1$
            }
            return data;
        }
        ...
        public void finish() throws IOException {
            if (!finished) {
                ...
                if (size() <= 0xffffffffL) {
                    ...
                    headerData.writeType("wide"); //$NON-NLS-1$
                    headerData.writeUInt(size());
                    headerData.writeType(type);
                } else {
    ...
    public void setTimeScale(int newValue) {
        if (newValue <= 0) {
            throw new IllegalArgumentException("timeScale must be greater 0"); //$NON-NLS-1$
        }
    ...
    public void setVideoDimension(int width, int height) {
        if (width < 1 || height < 1) {
            throw new IllegalArgumentException("width and height must be greater zero."); //$NON-NLS-1$
        }
        this.imgWidth = width;
    ...
    private void ensureStarted() throws IOException {
        if (state != States.STARTED) {
            ...
            writeProlog();
            mdatAtom = new WideDataAtom("mdat"); //$NON-NLS-1$
    ...
    public void writeFrame(BufferedImage image, int duration) throws IOException {
        if (duration <= 0) {
            throw new IllegalArgumentException("duration must be greater 0"); //$NON-NLS-1$
        }
        ensureOpen();
        ensureStarted();
        ...
        if (imgWidth == -1) {
            ...
            if (imgWidth != image.getWidth() || imgHeight != image.getHeight()) {
                throw new IllegalArgumentException("Dimensions of image[" + videoFrames.size() + //$NON-NLS-1$
                        "] (width=" + image.getWidth() + ", height=" + image.getHeight() + //$NON-NLS-1$ //$NON-NLS-2$
                        ") differs from image[0] (width=" + //$NON-NLS-1$
                        imgWidth + ", height=" + imgHeight); //$NON-NLS-1$
            }
        }
        ...
        switch (videoFormat) {
            ...
            case JPG: {
                ImageWriter iw = (ImageWriter) ImageIO.getImageWritersByMIMEType("image/jpeg").next(); //$NON-NLS-1$
            ...
            case PNG:
            default: {
                ImageWriter iw = (ImageWriter) ImageIO.getImageWritersByMIMEType("image/png").next(); //$NON-NLS-1$
                ImageWriteParam iwParam = iw.getDefaultWriteParam();
                MemoryCacheImageOutputStream imgOut = new MemoryCacheImageOutputStream(mdatAtom.getOutputStream());
                iw.setOutput(imgOut);
                IIOImage img = new IIOImage(image, null, null);
    ...
    public void finish() throws IOException {
        ...
        if (state != States.FINISHED) {
            if (imgWidth == -1 || imgHeight == -1) {
                throw new IllegalStateException("image width and height must be specified"); //$NON-NLS-1$
            }
            mdatAtom.finish();
            writeEpilog();
    ...
    private void writeEpilog() throws IOException {
        ...
        /* Movie Atom ========= */
        CompositeAtom moovAtom = new CompositeAtom("moov"); //$NON-NLS-1$
         ...
         */
        leaf = new DataAtom("mvhd"); //$NON-NLS-1$
        moovAtom.add(leaf);
        ...
        /* Track Atom ======== */
        CompositeAtom trakAtom = new CompositeAtom("trak"); //$NON-NLS-1$
        ...
        leaf = new DataAtom("tkhd"); //$NON-NLS-1$
        trakAtom.add(leaf);
        ...
        /* Media Atom ========= */
        CompositeAtom mdiaAtom = new CompositeAtom("mdia"); //$NON-NLS-1$
        ...
        leaf = new DataAtom("mdhd"); //$NON-NLS-1$
        mdiaAtom.add(leaf);
        d = leaf.getOutputStream();
        d.write(0); // version
        ...
        /** Media Handler Atom ------- */
        leaf = new DataAtom("hdlr"); //$NON-NLS-1$
        mdiaAtom.add(leaf);
        /*typedef struct {
        byte version;
        byte[3] flags;
        ...
        d.writeType("mhlr"); // componentType //$NON-NLS-1$
        ...
        d.writeType("vide"); // componentSubtype //$NON-NLS-1$
        ...
        CompositeAtom minfAtom = new CompositeAtom("minf"); //$NON-NLS-1$
        mdiaAtom.add(minfAtom);

        /* Video media information atom -------- */
        leaf = new DataAtom("vmhd"); //$NON-NLS-1$
        minfAtom.add(leaf);
        /*typedef struct {
        byte version;
        byte flag1;
        ...
        d.writeType("dhlr"); // componentType //$NON-NLS-1$
        // A four-character code that identifies the type of the handler. Only
        // two values are valid for this field: 'mhlr' for media handlers and 
        // 'dhlr' for data handlers.

        d.writeType("alis"); // componentSubtype //$NON-NLS-1$
        // A four-character code that identifies the type of the media handler 
        // or data handler. For media handlers, this field defines the type of
        // data—for example, 'vide' for video data or 'soun' for sound data.
        // For data handlers, this field defines the data reference type—for 
        ...
        CompositeAtom dinfAtom = new CompositeAtom("dinf"); //$NON-NLS-1$
        ...
        leaf = new DataAtom("dref"); //$NON-NLS-1$
        dinfAtom.add(leaf);
        /*typedef struct {
        ubyte version;
        ubyte[3] flags;
        ...
        d.writeType("alis"); // dataReference.type //$NON-NLS-1$
        ...
        /* Sample Table atom ========= */
        CompositeAtom stblAtom = new CompositeAtom("stbl"); //$NON-NLS-1$
        ...
        leaf = new DataAtom("stsd"); //$NON-NLS-1$
        stblAtom.add(leaf);
        /*
        typedef struct {
        byte version;
        ...
        switch (videoFormat) {
            case RAW: {
                ...
                d.writeType("raw "); // sampleDescriptionTable[0].type //$NON-NLS-1$
                ...
                d.writeType("java"); // sampleDescriptionTable.videoSampleDescription.manufacturer //$NON-NLS-1$
                ...
                d.writePString("None", 32); // sampleDescriptionTable.videoSampleDescription.compressorName //$NON-NLS-1$
            ...
            case JPG: {
                d.writeInt(86); // sampleDescriptionTable[0].size
                d.writeType("jpeg"); // sampleDescriptionTable[0].type //$NON-NLS-1$
                ...
                d.writePString("Photo - JPEG", 32); // sampleDescriptionTable.videoSampleDescription.compressorName //$NON-NLS-1$
                // A 32-byte Pascal string containing the name of the compressor 
                // that created the image, such as "jpeg".
            ...
            case PNG: {
                d.writeInt(86); // sampleDescriptionTable[0].size
                d.writeType("png "); // sampleDescriptionTable[0].type //$NON-NLS-1$
                // A 32-bit integer indicating the format of the stored data.
                // This depends on the media type, but is usually either the 
                // compression format or the media type.
                ...
                d.writePString("PNG", 32); // sampleDescriptionTable.videoSampleDescription.compressorName //$NON-NLS-1$
                // A 32-byte Pascal string containing the name of the compressor 
                // that created the image, such as "jpeg".
        ...
        // of 'stts'.
        leaf = new DataAtom("stts"); //$NON-NLS-1$
        stblAtom.add(leaf);
        /*
        typedef struct {
        byte version;
        ...
        leaf = new DataAtom("stsc"); //$NON-NLS-1$
        stblAtom.add(leaf);
        /*
        typedef struct {
        byte version;
        ...
        // no table follows.
        leaf = new DataAtom("stsz"); //$NON-NLS-1$
        stblAtom.add(leaf);
        /*
        typedef struct {
        byte version;
        ...
        if (videoFrames.size() == 0 || videoFrames.getLast().offset <= 0xffffffffL) {
            ...
            leaf = new DataAtom("stco"); //$NON-NLS-1$
            stblAtom.add(leaf);
            /*
            typedef struct {
            byte version;
            ...
            leaf = new DataAtom("co64"); //$NON-NLS-1$
            stblAtom.add(leaf);
            /*
            typedef struct {
            byte version;
