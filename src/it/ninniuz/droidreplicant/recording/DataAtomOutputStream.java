...
public class DataAtomOutputStream extends FilterOutputStream {
    ...
    public void writeType(String s) throws IOException {
        if (s.length() != 4) {
            throw new IllegalArgumentException("type string must have 4 characters"); //$NON-NLS-1$
        }

        try {
            out.write(s.getBytes("ASCII"), 0, 4); //$NON-NLS-1$
            incCount(4);
    ...
    public void writePString(String s) throws IOException {
        if (s.length() > 0xffff) {
            throw new IllegalArgumentException("String too long for PString"); //$NON-NLS-1$
        }
        if (s.length() < 256) {
            out.write(s.length());
        } else {
    ...
    public void writePString(String s, int length) throws IOException {
        if (s.length() > length) {
            throw new IllegalArgumentException("String too long for PString of length "+length); //$NON-NLS-1$
        }
        if (s.length() < 256) {
            out.write(s.length());
        } else {
